...
const rows = countries.map(([iso2, name, dialCode]) => {
  const safeName = name.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
  return `  { iso2: '${iso2}', name: '${safeName}', dialCode: '${dialCode}', flag: '${flag(iso2)}' },`;
});

const out = `/** UI-only country dial codes for auth phone picker (no backend coupling). */
...
const dest = path.join(__dirname, "..", "src", "mocks", "data", "country-dial-codes.ts");
